...
export class OgcExportModal {
  static show(metadata, instruments = []) {
    ...
    modalRoot.innerHTML = `
      <div id="export-backdrop" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fadeIn">
        <div class="relative w-full max-w-3xl glass-panel rounded-2xl p-6 text-white shadow-2xl border border-slate-700 max-h-[90vh] flex flex-col overflow-hidden">
          
          <!-- Header -->
          <div class="flex items-center justify-between pb-3 border-b border-slate-800 shrink-0">
            <div class="flex items-center gap-3">
              <div class="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xl">
                🌐
              </div>
              <div>
                <h3 class="font-bold text-sm tracking-wide text-white">Open Standards & Interoperability Hub</h3>
                <p class="text-[11px] text-slate-400">OGC WMS/WCS Services, CF-1.8 Conventions & GeoJSON In-Situ Export</p>
              </div>
            </div>
            <button id="btn-close-export" class="p-1.5 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition cursor-pointer text-lg">
              ✕
            </button>
          </div>

          <!-- Tabs -->
          <div class="flex border-b border-slate-800 my-3 gap-2 shrink-0">
            <button id="tab-btn-wms" class="px-3 py-1.5 rounded-t-lg font-bold text-xs bg-slate-800 text-emerald-400 border-b-2 border-emerald-400 cursor-pointer">
              OGC WMS 1.3.0 XML
            </button>
            <button id="tab-btn-geojson" class="px-3 py-1.5 rounded-t-lg font-bold text-xs text-slate-400 hover:text-white cursor-pointer">
              In-Situ GeoJSON (${instruments.length} Platforms)
            </button>
            <button id="tab-btn-cf" class="px-3 py-1.5 rounded-t-lg font-bold text-xs text-slate-400 hover:text-white cursor-pointer">
              CF-1.8 Metadata Report
            </button>
          </div>

          <!-- Code View Area -->
          <div class="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-800 overflow-y-auto font-mono text-[11px] text-slate-300 custom-scrollbar relative">
          ...
          <div class="mt-4 flex items-center justify-between pt-3 border-t border-slate-800 shrink-0">
            <span class="text-[11px] text-slate-400 font-mono">Complies with OGC, ISO-19115 & CF-1.8 NetCDF</span>
