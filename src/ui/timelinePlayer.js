export class TimelinePlayer {
  ...
  render() {
    this.container.innerHTML = `
      <div class="flex items-center gap-1.5 sm:gap-3 bg-slate-900/95 backdrop-blur-xl px-3 py-1.5 sm:px-5 sm:py-2 rounded-full border border-slate-700/80 shadow-2xl select-none max-w-full overflow-x-auto">
        <!-- Step Back -->
        <button id="btn-prev" class="p-1 sm:p-1.5 rounded-full hover:bg-slate-800 text-slate-400 hover:text-white transition cursor-pointer" title="Previous Step">
          <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
        </button>

        <!-- Play / Pause -->
        <button id="btn-play" class="flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-4 sm:py-1.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold rounded-full text-[11px] sm:text-xs transition shadow-md cursor-pointer transform active:scale-95">
          <svg id="play-icon" class="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-current" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
          <span id="play-text">Play</span>
        </button>

        <!-- Step Next -->
        <button id="btn-next" class="p-1 sm:p-1.5 rounded-full hover:bg-slate-800 text-slate-400 hover:text-white transition cursor-pointer" title="Next Step">
          <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
        </button>
  ...
  bindEvents() {
    ...
    playBtn.addEventListener('click', () => {
      ...
      if (this.isPlaying) {
        playBtn.className = 'flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-4 sm:py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-full text-[11px] sm:text-xs transition shadow-md cursor-pointer transform active:scale-95';
        ...
        playBtn.className = 'flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 sm:px-4 sm:py-1.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold rounded-full text-[11px] sm:text-xs transition shadow-md cursor-pointer transform active:scale-95';
